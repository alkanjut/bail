"use strict";
const NodeCache = require('@cacheable/node-cache').default || require('@cacheable/node-cache');
const { Boom } = require('@hapi/boom');
const { proto } = require('../../WAProto/index.js');
const { DEFAULT_CACHE_TTLS, WA_DEFAULT_EPHEMERAL } = require('../Defaults/index.js');
const { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, unixTimestampSeconds, setBotMessageSecret } = require('../Utils/index.js');
const { getUrlInfo } = require('../Utils/link-preview.js');
const { makeKeyedMutex, makeMutex } = require('../Utils/make-mutex.js');
const { getMessageReportingToken, shouldIncludeReportingToken } = require('../Utils/reporting-utils.js');
const { buildMergedTcTokenIndexWrite, isTcTokenExpired, resolveIssuanceJid, resolveTcTokenJid, shouldSendNewTcToken, storeTcTokensFromIqResult } = require('../Utils/tc-token-utils.js');
const { areJidsSameUser, getBinaryNodeChild, getBinaryNodeChildren, isHostedLidUser, isHostedPnUser, isJidBot, isJidGroup, isJidMetaAI, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, PSA_WID, S_WHATSAPP_NET, getAdditionalNode, getBinaryNodeFilter, getBinaryFilteredBizBot, isInteropUser } = require('../WABinary/index.js');
const { USyncQuery, USyncUser } = require('../WAUSync/index.js');
const { makeNewsletterSocket } = require('./newsletter.js');
const imup = require('./luxu.js').default || require('./luxu.js');
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                if (messageType) {
                    switch(messageType) {
                        case 'PAYMENT':
                            let paymentContent = await luki.handlePayment(content, quoted);
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID()
                            });
                        case 'PRODUCT':
                            let productContent = await luki.handleProduct(content, jid, quoted);
                            let productMsg = await Utils_1.generateWAMessageFromContent(jid, productContent, { quoted });
                            return await relayMessage(jid, productMsg.message, {
                                messageId: productMsg.key.id,
                            });
                        case 'INTERACTIVE':
                            let interactiveContent = await luki.handleInteractive(content, jid, quoted);
                            let interactiveMsg = await Utils_1.generateWAMessageFromContent(jid, interactiveContent, { quoted });
